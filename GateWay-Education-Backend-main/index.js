...
app.use(
  cors({
    origin: ["https://localhost:5173", "http://localhost:5173", "http://localhost:4173"],
    methods: ["GET", "POST", "PUT", "DELETE", "UPDATE"],
    credentials: true,
  })
);
...
// ✅ General Rate Limiter (100 requests per 1 min)
const limiter = rateLimit({
  windowMs: 60 * 1000,
  max: 100,
  message: "Too many requests. Please slow down.",
...
// ✅ Forgot Password Request Limiter
...
app.use("/auth/reset-password", rateLimit({
    windowMs: 30 * 60 * 1000,
    max: 5,
    message: "Too many reset attempts. Try again later.",
    standardHeaders: true,
