...
const ManageStudents = () => {
  ...
  const handleSave = async () => {
    if (originalRole === "Admin" && selectedUser.role === "Student") {
      toast.error("❌ You cannot change an Admin to a Student.");
      return;
    }

    try {
      await axios.put(`/user/update/${selectedUser._id}`, selectedUser, {
        headers: {
  ...
  const handleDelete = async (id, role) => {
    setSelectedUser(null); // prevent edit modal triggering
  ...
  {
    icon: false, // ✅ disables the default green check
  }
);
    } catch (error) {
                    ...
                    <div className="grid grid-cols-12 gap-4 items-center">
                      ...
                      <div className="col-span-3">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <h3 className="text-sm font-semibold text-gray-900 max-w-[160px] truncate" title={user.name}>
                              {user.name}
                            </h3>
                      ...
                      <div className="col-span-4">
                        <p className="text-sm text-gray-900 max-w-[240px] truncate" title={user.email}>
                          {user.email}
                        </p>
...
// ✅ All the other sub-components remain unchanged
      ...
      <div className="p-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              type="text"
              className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white text-gray-900 placeholder-gray-500"
              value={user.name}
        ...
        <div className="flex justify-end space-x-2 mt-6">
          <button onClick={onClose} className="px-5 py-2 bg-gray-100 text-gray-700 rounded-lg">Cancel</button>
          <button onClick={onSave} className="px-5 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">Save</button>
        </div>
      </div>
    </div>
  </div>
